...
1. **Reviewer cannot POST /reviews via curl under the literal allowlist.** Phase 6 step 3 specifies the reviewer's `--allowed-tools` as `Read,Grep,Glob,Bash(git diff:*,git log:*,wc:*,ls:*)` — which excludes `curl`. Phase 6 step 4 instructs the reviewer "Your last action before exiting is to POST your verdict and findings to ${SERVER_URL}/tasks/<task-id>/reviews". These two requirements are in tension: under the literal allowlist the reviewer cannot run `curl`, and Claude Code's built-in `WebFetch` does GET only. **I implemented the allowlist literally per the plan and the integration-note guidance ("The `--allowed-tools` allowlist is the security boundary")**, but in practice the reviewer's POST will be tool-not-allowed at runtime. The fanout's recovery loop will retry twice and then fail the task with `reviewer_infrastructure_failure`. Two ways to resolve, both above my pay grade: (a) extend the allowlist with `Bash(curl:*)` (relies on the inject-agent-header hook to add headers, retains read-only-source-file property), or (b) add a project-specific "post-review" MCP tool that proxies the POST. Surfacing as a blocking issue for the next planner — under the current literal allowlist the fanout will fail every task it processes.
