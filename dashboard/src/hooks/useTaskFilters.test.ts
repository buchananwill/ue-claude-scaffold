...
const VALID_SORT_COLUMNS = new Set<string>([
  "id",
  "priority",
  "status",
  "title",
...
function parseSortColumn(sort: string | undefined): SortColumn {
  if (sort && VALID_SORT_COLUMNS.has(sort))
    return sort as NonNullable<SortColumn>;
  return null;
}
...
describe("filterTasks", () => {
  ...
  it("filters by priority", () => {
    ...
    expect(result.every((t) => t.priority === 1 || t.priority === 3)).toBe(
      true,
    );
  });
  ...
  it("filters by multiple statuses", () => {
    ...
    expect(result).toHaveLength(2);
    expect(
      result.every((t) => t.status === "pending" || t.status === "failed"),
    ).toBe(true);
  });
...
describe("sortTasks", () => {
  ...
  it("does not mutate the original array", () => {
    ...
    expect(tasks.map((t) => t.priority)).toEqual(
      original.map((t) => t.priority),
    );
  });
...
describe("search param serialization", () => {
  it("serializes non-empty string set to comma-separated string", () => {
    expect(serializeStringSet(new Set(["pending", "completed"]))).toBe(
      "pending,complete",
    );
  });
