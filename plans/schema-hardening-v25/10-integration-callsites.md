...
4. For every caller that passed an agent name string to a query that now takes a UUID (e.g. `tasksLifecycleQ.releaseByAgent`, `coalesceQ.pausePumpAgents`, `ubtQ.*`, `roomsQ.addMember`), add a `getByName` lookup at the call site to convert the name to a UUID before the call. If the caller already has the agent row from a prior lookup, reuse it. **UBT callers:** `ubtQ.*` functions no longer take `projectId` (UBT is host-level). Remove `request.projectId` from all UBT call sites in `routes/ubt.ts`. The `clearLockAndPromote` helper and `sweepStaleLock` already called without projectId in practice — now the signatures match.
...
- No file under `server/src/` (excluding tests) references `agentsQ.getProjectId`, `agentsQ.hardDelete`, `agentsQ.deleteAll`, or any of the old agent-name string parameters on task/file/coalesce/ubt/room functions. UBT functions take no `projectId` parameter.
