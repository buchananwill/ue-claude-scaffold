...
4. `server/src/queries/ubt.ts` — UBT is host-level, not project-scoped. Remove all `projectId` parameters from every function. Grep the file for references to `ubtLock.holder` and `ubtQueue.agent` — rename column references to `ubtLock.holderAgentId` and `ubtQueue.agentId`. Update all query writers and readers. The 60-second stale-lock sweep (referenced in `CLAUDE.md`) uses a JOIN on `agents` — ensure the JOIN is on `agents.id = ubt_lock.holder_agent_id`, not on name. Every function that operates on a specific agent takes `agentId: string` (UUID), not `agent: string`. The lock is keyed by `ubtLock.hostId` (default `'local'`); `acquireLock` uses `ON CONFLICT (host_id)` for the upsert. `getLock`, `releaseLock`, `dequeue`, `getQueue`, `findInQueue` take no project parameter — the lock and queue are global. `enqueue` takes `agentId` and `priority`, no `projectId`.
...
- `server/src/queries/ubt.ts` — all references to `ubt_lock.holder` and `ubt_queue.agent` are gone; JOINs use `agents.id = ubt_lock.holder_agent_id`; no function takes a `projectId` parameter (UBT is host-level); lock keyed by `hostId`.
