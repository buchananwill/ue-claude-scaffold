...
6. `server/src/routes/tasks.test.ts`, `build.test.ts`, `ubt.test.ts`, `coalesce.test.ts` — set `X-Project-Id` explicitly on requests. Register agents via the `/agents/register` route (not via Drizzle inserts) so test agents have valid UUIDs.
