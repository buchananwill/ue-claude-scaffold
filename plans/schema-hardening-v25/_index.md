...
**UBT lock is host-level, not project-scoped.** The UBT (Unreal Build Tool) lock guards a shared host resource — the build tool binary and its own internal mutex. The correct model is: a pool of mutex-locked host resources (current pool size: 1), with a global queue managing contention across all projects and agents. `ubt_lock` is keyed by `host_id` (default `'local'`), not `project_id`. `ubt_queue` has no `project_id` column — all agents compete in one queue. Neither table gets a `project_id` FK to `projects`. The agent identity FK (`holder_agent_id`, `agent_id` → `agents.id`) is correct and stays. A future distributed architecture may introduce a `ubt_hosts` registry table; the `host_id` PK is the extension point for that.
...
- **Live-state orphans** (`tasks.claimed_by`, `files.claimant`): NULL out the claim. The referenced work becomes unclaimed and re-queueable. No data loss that matters — these are transient claims.
- **UBT lock/queue orphans** (`ubt_lock.holder`, `ubt_queue.agent`): UBT is host-level, not project-scoped. Clear the lock table entirely (transient state, re-acquired on next build). Backfill `ubt_queue.agent_id` by name match (no project join); delete unresolvable entries. Neither table participates in the `project_id` orphan cleanup pass.
...
6. [Agent-referencing query layer](06-agent-referencing-queries.md) — rewrite `tasks-lifecycle.ts`, `files.ts`, `coalesce.ts` to take `projectId` and `agentId`; rewrite `ubt.ts` to take `agentId` only (UBT is host-level, no `projectId`).
...
### Operator phases (post-merge, not executed by containers)

15. [Apply migration to local DB](15-operator-apply-migration.md) — snapshot data dir, run `db:migrate`, verify post-migration state, restart server.
16. [Operator rebuild and smoke test](16-operator-smoke-test.md) — `docker compose build`, snapshot, launch two same-named agents in different projects, verify cross-project isolation and chat protocol end-to-end.

## Execution model

Phases 1–13 are executed by container agents. The container works against the Drizzle schema declarations in `tables.ts` and validates via ephemeral PGlite instances (test helper) — it does not touch the host's live DB. The host server continues running on the old (pre-migration) schema throughout, using the old column names that still exist in the DB.

Phases 15–16 are operator-only, executed after all container work is merged. The operator backs up the PGlite data dir, runs `db:migrate` (which applies 0002+0003+0004 atomically), restarts the server on the new code, and runs the smoke test. There is no Phase 4 — migration was intentionally removed from the container execution sequence.
...
- Every project-scoped data table's `project_id` is a FK on `projects.id` with `ON DELETE RESTRICT`. UBT tables (`ubt_lock`, `ubt_queue`) are host-level and have no `project_id` FK; `ubt_lock` is keyed by `host_id`, `ubt_queue` has no `project_id` column.
