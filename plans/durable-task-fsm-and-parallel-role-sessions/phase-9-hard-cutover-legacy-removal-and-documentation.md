...
- **Schema-shape smoke tests** (run after the migration is applied; confirm CHECK / UNIQUE behaviour):
  - `INSERT INTO tasks (project_id, title, status) VALUES ('ue-claude-scaffold','probe','engineering')` succeeds; `... ('ue-claude-scaffold','probe','garbage')` fails the status CHECK; `... ('ue-claude-scaffold','probe','in_progress')` fails the status CHECK (legacy value rejected).
  - `INSERT INTO review_runs (task_id, cycle, reviewer_role, verdict, raw_markdown) VALUES (<probe_id>, 1, 'safety', 'approve', '')` succeeds; a second insert with the same `(task_id, cycle, reviewer_role)` triple fails the unique constraint.
  - `INSERT INTO arbitration_runs (task_id, trigger, ruling, ruling_markdown) VALUES (<probe_id>, 'review_cycle_budget_exhausted', 'approve', '')` succeeds; a second insert with the same `(task_id, trigger)` pair fails the unique constraint; an insert with `ruling='rule'` and `contradiction_resolution = NULL` fails the rule-resolution CHECK; with a populated `contradiction_resolution`, succeeds.
  - The application-layer Zod validator (Phase 1 step 2a, applied in `server/src/config-resolver.ts` and `server/src/routes/tasks-ingest.ts`) rejects an `agentRoles` jsonb that is missing `engineer`, missing `arbitrator`, missing `reviewers`, has an empty `reviewers`, has a reviewer-role slug containing uppercase, or has an unknown top-level key. With all required keys and valid slugs, validation passes.
