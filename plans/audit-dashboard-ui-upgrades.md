...
**Scope:** Independent UI/UX improvements to dashboard. No coordination server changes. Can be merged in parallel or
independently from server migration.
...
- **Impact:** Reduces usability; hard to share specific chat context
...
- **Impact:** Frustrating UX when monitoring progress; forces re-reading
...
- **Impact:** Hard to read structured reports; aesthetic regression
...
**Recommendation:** Use **AI Elements `<MessageResponse>`** if available (aligns with chat SDK philosophy). Otherwise,
`react-markdown` + `highlight.js` for code.
