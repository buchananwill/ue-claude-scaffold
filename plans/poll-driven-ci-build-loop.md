...
- Run every git child in `ciSyncWorktree` with a non-interactive environment so a missing/expired credential fails fast instead of blocking on a prompt: set `GIT_TERMINAL_PROMPT=0` and `GIT_SSH_COMMAND="ssh -o BatchMode=yes"` in the child env. A fetch that cannot authenticate then returns a git error within the timeout, which `runCiRequest` surfaces and the loop records as `status='error'` — it must never hang waiting for input that will never arrive on the unattended host.
...
**Work:**
- Provision the dedicated CI worktree once on the host: `git clone <github-url> <ciWorktreePath>` so its `origin` is GitHub. Authenticate it with a **non-interactive, read-only** credential — a passphraseless read-only SSH deploy key (clone via `git@github.com:…`) is preferred; a fine-grained PAT scoped to Contents-read with expiry past the trip is the HTTPS fallback. Do not leave it on a Git Credential Manager OAuth credential. Set `server.ciWorktreeRoot` (or the per-project `ciWorktreePath`) in `scaffold.config.json`.
- **Pre-departure litmus test (do this before leaving, from a clean shell):** `GIT_TERMINAL_PROMPT=0 GIT_SSH_COMMAND="ssh -o BatchMode=yes" git -C <ciWorktreePath> fetch origin <branch>` must exit 0 with no prompt. If it prompts or fails, the credential is interactive/expired and the loop would stall mid-week — fix it now, not in Folkestone. Also confirm `git -C <ciWorktreePath> remote get-url origin` points at GitHub.
