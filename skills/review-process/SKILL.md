...
3. If a changed function takes or returns a pointer/reference, `Grep` for that symbol to understand its lifecycle across
   call sites

Adapt depth to your mandate: style review may be self-contained per file; safety and correctness review require
cross-file tracing.

### Step 3: Check Against Domain Criteria

Systematically check every rule from your loaded domain skill(s). For each file, for each rule category — be thorough
but precise. Only flag things that are clearly violations, not judgment calls.

### Step 4: Score and Filter

Rate every potential finding on a confidence scale. Apply the scoring thresholds and output format from your output
schema. Only report issues you can substantiate with specific code evidence.
