...
describe("files queries", () => {
  ...
  before(async () => {
    ...
    const a1 = await agentQ.register(db, {
      name: "agent-1",
      worktree: "w1",
      projectId: "default",
      sessionToken: "fl-tok-1",
    });
    const a2 = await agentQ.register(db, {
      name: "agent-2",
      worktree: "w2",
      projectId: "default",
      sessionToken: "fl-tok-2",
    });
    agent1Id = a1.id;
    agent2Id = a2.id;
