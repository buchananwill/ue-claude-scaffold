...
describe("tasks-core queries", () => {
  ...
  it("should patch a pending task", async () => {
    ...
    const ok = await tasksCore.patch(db, task.id, {
      title: "Patched",
      priority: 10,
    });
  ...
  it("should not patch a non-pending task", async () => {
    ...
    await db.execute(
      sqlTag`UPDATE tasks SET status = 'claimed' WHERE id = ${task.id}`,
    );
  ...
  it("should not delete a task in any FSM mid-state", async () => {
    ...
    for (const fsmStatus of ["engineering", "reviewing"] as const) {
      ...
      await db.execute(
        sqlTag`UPDATE tasks SET status = ${fsmStatus} WHERE id = ${task.id}`,
      );
      const ok = await tasksCore.deleteById(db, task.id);
