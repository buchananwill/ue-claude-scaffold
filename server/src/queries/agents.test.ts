...
describe("agents queries", () => {
  ...
  before(async () => {
    ...
    await db
      .insert(projects)
      .values({ id: "proj-x", name: "Project X" })
      .onConflictDoNothing();
  });
  ...
  it("should deleteAllForProject (soft-delete all non-deleted)", async () => {
    await agentQ.register(db, {
      ...
      worktree: "w1",
      projectId: "default",
      sessionToken: "tok-del-a1",
    });
    await agentQ.register(db, {
      name: "del-a2",
      worktree: "w2",
      projectId: "default",
      sessionToken: "tok-del-a2",
    });
  ...
  it("should getByNameFull return sessionToken", async () => {
    const reg = await agentQ.register(db, {
      name: "full-test",
      worktree: "wt",
      sessionToken: "tok-full",
      projectId: "default",
    });
