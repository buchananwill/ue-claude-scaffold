...
describe("tasks-claim queries", () => {
  ...
  describe("claimNextCandidate", () => {
    ...
    before(async () => {
      ...
      const r1 = await tasksCore.insert(db, {
        title: "T1 high prio",
        priority: 10,
        projectId: proj,
      });
      const r2 = await tasksCore.insert(db, {
        title: "T2 dep on T1",
        priority: 5,
        projectId: proj,
      });
      const r3 = await tasksCore.insert(db, {
        title: "T3 file conflict",
        priority: 8,
        projectId: proj,
      });
      const r4 = await tasksCore.insert(db, {
        title: "T4 low prio",
        priority: 3,
        projectId: proj,
      });
      const r5 = await tasksCore.insert(db, {
        title: "T5 blocked dep",
        priority: 5,
        projectId: proj,
      });
      const r6 = await tasksCore.insert(db, {
        title: "T6 lowest",
        priority: 1,
        projectId: proj,
      });
      ...
      // T1 is completed by agent-a. The dep-satisfaction query uses
      // claimed_by_agent_id (UUID) under the FSM — the legacy result.agent
      // JSON column is no longer populated and no longer consulted.
      await db.execute(
        sql`UPDATE tasks SET status = 'completed', claimed_by_agent_id = ${agentAId}, claimed_at = now() WHERE id = ${t1}`,
      );
    ...
    it("should return candidates in correct order for agent-a", async () => {
      const candidates = await tasksClaim.claimNextCandidate(
        db,
        "claim-proj",
        agentAId,
      );
      ...
      assert.ok(
        candidates.length >= 3,
        `Expected >= 3 candidates, got ${candidates.length}`,
      );
      ...
      assert.equal(
        candidates[0].id,
        t2,
        "T2 should be first due to agent affinity",
      );
  ...
  describe("count functions", () => {
    ...
    it("should count file-blocked tasks", async () => {
      ...
      const agentARow = await db.execute(
        sql`SELECT id FROM agents WHERE name = 'agent-a' AND project_id = 'claim-proj'`,
      );
    ...
    it("should count dep-blocked tasks", async () => {
      // Re-query agent UUIDs — this describe block is a sibling of the
      // claimNextCandidate block where they were declared, so they're out
      // of scope here.
      const agentARow = await db.execute(
        sql`SELECT id FROM agents WHERE name = 'agent-a' AND project_id = 'claim-proj'`,
      );
      const agentBRow = await db.execute(
        sql`SELECT id FROM agents WHERE name = 'agent-b' AND project_id = 'claim-proj'`,
      );
      const agentAId = (agentARow.rows[0] as { id: string }).id;
      const agentBId = (agentBRow.rows[0] as { id: string }).id;

      // T5 has unmet dep (T6 is pending)
      // T2's dep (T1) is completed by agent-a — for agent-a, T2 is NOT dep-blocked
      const c = await tasksClaim.countDepBlocked(db, "claim-proj", agentAId);
      assert.equal(c, 1); // T5

      // For agent-b, T2's dep (T1) is completed by agent-a, not agent-b — so T2 IS dep-blocked
      const c2 = await tasksClaim.countDepBlocked(db, "claim-proj", agentBId);
