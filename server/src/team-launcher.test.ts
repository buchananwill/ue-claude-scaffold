...
function createTestBareRepo(
  ...
  execSync(
    `git update-ref refs/heads/docker/${projectId}/current-root ${sha}`,
    {
      cwd: bareDir,
      stdio: "pipe",
    },
...
describe("loadTeamDef", () => {
  ...
  it("throws when no leader is present", () => {
    const dir = createTeamsDir("no-leader", {
      id: "no-leader",
      name: "No Leader",
      members: [{ agentName: "a", role: "impl", agentType: "implementer" }],
    });
    assert.throws(
...
describe("launchTeam", () => {
  ...
  it("happy path: registers team, posts brief, sets branches, returns plan", async () => {
    ...
    const messages = await chatQ.getHistory(testDb.db, "my-team", {
      limit: 10,
    });
  ...
  it("resolves hooks with team-level defaults and member overrides", async () => {
    ...
    // Leader overrides buildIntercept to true, inherits team cppLint=false, jsLint defaults false
    ...
    assert.equal(leader.hooks.jsLint, false);
    ...
    assert.equal(worker.hooks.jsLint, false);
