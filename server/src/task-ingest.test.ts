...
describe("ingestTaskFile", () => {
  ...
  it("happy path: parses all frontmatter fields", async () => {
    ...
    const result = await ingestTaskFile(
      ctx.db,
      "/tasks/build-widget.md",
      content,
      "default",
    );
    ...
    assert.equal(
      ...
      "Widget compiles and passes tests",
    );
    ...
    const fileRows = await ctx.db
      .select()
      .from(taskFiles)
      .where(eq(taskFiles.taskId, result.taskId));
  ...
  it("title falls back to filename when frontmatter has no title", async () => {
    const content = ["---", "priority: 3", "---", "Some description."].join(
      "\n",
    );
  ...
  it("dedup on re-ingest: same sourcePath returns skipped", async () => {
    const content = ["---", "title: Duplicate task", "---", "Body."].join("\n");

    const first = await ingestTaskFile(
      ctx.db,
      "/tasks/dup.md",
      content,
      "default",
    ...
    const second = await ingestTaskFile(
      ctx.db,
      "/tasks/dup.md",
      content,
      "default",
  ...
  it("malformed frontmatter (no --- delimiters) treats entire content as description", async () => {
    ...
    assert.ok(
      rows[0].description!.includes("This is just a plain markdown file"),
    );
  });

  it("different projectId does not dedup", async () => {
    const content = ["---", "title: Cross-project task", "---", "Body."].join(
      "\n",
    );
  ...
  it("priority defaults to 0 when not specified", async () => {
    const content = ["---", "title: No priority", "---", "Body."].join("\n");

    const result = await ingestTaskFile(
      ctx.db,
      "/tasks/no-priority.md",
      content,
      "default",
    );
    const rows = await ctx.db
...
describe("ingestTaskDir", () => {
  ...
  it("ingests .md files, skips non-.md files", async () => {
    ...
    assert.ok(
      result.replanned >= 0,
      "replanned should be returned from runReplan",
    );
  ...
  it("dedup on re-ingest returns skipped for existing tasks", async () => {
    await writeFile(
      join(tmpDir, "repeat.md"),
      ["---", "title: Repeat Task", "---", "Body."].join("\n"),
    );
