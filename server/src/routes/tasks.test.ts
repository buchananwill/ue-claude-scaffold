...
describe("tasks routes", () => {
  ...
  it("POST /tasks creates a task and returns id", async () => {
    const res = await ctx.app.inject({
      ...
      payload: {
        title: "Build widget",
        description: "Create the widget system",
      },
      headers: { "x-project-id": "default" },
    });
  ...
  it("GET /tasks with multi-status filter", async () => {
    ...
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "Pending1" },
      headers: { "x-project-id": "default" },
    });
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "Pending2" },
      headers: { "x-project-id": "default" },
    });
    ...
    const res2 = await ctx.app.inject({
      method: "GET",
      url: "/tasks?status=pending,complete",
      headers: { "x-project-id": "default" },
    });
  ...
  it("GET /tasks with priority filter", async () => {
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "P0", priority: 0 },
      headers: { "x-project-id": "default" },
    });
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "P1", priority: 1 },
      headers: { "x-project-id": "default" },
    });
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "P2", priority: 2 },
      headers: { "x-project-id": "default" },
    });

    const res = await ctx.app.inject({
      method: "GET",
      url: "/tasks?priority=0,2",
      headers: { "x-project-id": "default" },
    });
    const body = res.json() as TaskListBody;
    assert.equal(body.tasks.length, 2);
    assert.equal(body.total, 2);
    const priorities = body.tasks.map(
      (t: Record<string, unknown>) => t.priority,
    );
  ...
  it("GET /tasks with sort and dir", async () => {
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "AAA", priority: 1 },
      headers: { "x-project-id": "default" },
    });
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "ZZZ", priority: 2 },
      headers: { "x-project-id": "default" },
    });
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "MMM", priority: 0 },
      headers: { "x-project-id": "default" },
    });
  ...
  it("GET /tasks with invalid dir returns 400", async () => {
    const res = await ctx.app.inject({
      method: "GET",
      url: "/tasks?sort=title&dir=sideways",
      headers: { "x-project-id": "default" },
    });
    assert.equal(res.statusCode, 400);
    const body = res.json();
  ...
  it("GET /tasks with agent filter", async () => {
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "Unassigned1" },
      headers: { "x-project-id": "default" },
    });
    await ctx.app.inject({
      method: "POST",
      url: "/tasks",
      payload: { title: "Unassigned2" },
      headers: { "x-project-id": "default" },
    });
    ...
    const res = await ctx.app.inject({
      method: "GET",
      url: "/tasks?agent=__unassigned__",
      headers: { "x-project-id": "default" },
    });
    const body = res.json() as TaskListBody;
    assert.equal(body.tasks.length, 2);
  ...
  it("GET /tasks?claimedByAgentId=<valid-uuid> returns only tasks claimed by that agent", async () => {
    ...
    const res = await ctx.app.inject({
      method: "GET",
      url: `/tasks?claimedByAgentId=${agentId}`,
      headers: { "x-project-id": "default" },
    });
    assert.equal(res.statusCode, 200);
    const body = res.json() as TaskListBody;
  ...
  it("GET /tasks?claimedByAgentId=<malformed> returns 400", async () => {
    const res = await ctx.app.inject({
      method: "GET",
      url: "/tasks?claimedByAgentId=not-a-uuid",
      headers: { "x-project-id": "default" },
    });
    assert.equal(res.statusCode, 400);
  ...
  it("GET /tasks?claimedByAgentId=<unknown-uuid> returns empty list", async () => {
    ...
    const res = await ctx.app.inject({
      method: "GET",
      url: `/tasks?claimedByAgentId=${phantomUuid}`,
      headers: { "x-project-id": "default" },
    });
    assert.equal(res.statusCode, 200);
    const body = res.json() as TaskListBody;
  ...
  it("POST /tasks/batch creates all tasks atomically with file registrations", async () => {
    ...
    const get1 = await ctx.app.inject({
      method: "GET",
      url: `/tasks/${body.ids[0]}`,
      headers: { "x-project-id": "default" },
    });
    ...
    const get2 = await ctx.app.inject({
      method: "GET",
      url: `/tasks/${body.ids[1]}`,
      headers: { "x-project-id": "default" },
    });
    assert.deepEqual(get2.json().files.sort(), ["Only2.h", "Shared.cpp"]);

    const get3 = await ctx.app.inject({
      method: "GET",
      url: `/tasks/${body.ids[2]}`,
      headers: { "x-project-id": "default" },
    });
  ...
  it("batch with ?replan=true returns replan summary", async () => {
    ...
    assert.ok(
      body.replan.replanned >= 3,
      `expected replanned >= 3, got ${body.replan.replanned}`,
    );
    ...
    const rootTask = await ctx.app.inject({
      method: "GET",
      url: `/tasks/${body.ids[0]}`,
      headers: { "x-project-id": "default" },
    });
    ...
    it("returns 409 for every FSM in-flight status (protected mid-states)", async () => {
      ...
      for (const status of fsmInFlight) {
        ...
        assert.equal(
          res.statusCode,
          409,
          `DELETE ?status=${status} should return 409`,
        );
        const body = res.json();
        assert.ok(
