...
describe("messages routes (drizzle)", () => {
  ...
  it("?since=<id>&limit=N caps to the NEWEST rows (tail-anchored) in polling mode", async () => {
    const ids: number[] = [];
    for (let i = 1; i <= 5; i++) {
      const r = await ctx.app.inject({
        method: "POST",
    ...
    // 4 rows match id > ids[0] (m2..m5); with limit=2 the cap returns the
    // freshest pair (m4, m5), not the oldest pair — a caller that fell behind
    // lands on the live tail rather than replaying stale history.
    const get = await ctx.app.inject({
      method: "GET",
    ...
    const msgs = get.json();
    assert.equal(msgs.length, 2);
    assert.equal(msgs[0].payload, "m4");
    assert.equal(msgs[1].payload, "m5");
    // Still ascending within the page.
    assert.ok(msgs[0].id < msgs[1].id);
  });

  it("?since=0 on a channel larger than the cap returns the freshest tail", async () => {
    // 6 messages, cap the page at 3: a naive one-shot since=0 must land on the
    // newest 3 (m4, m5, m6), not the oldest 3.
    for (let i = 1; i <= 6; i++) {
      await ctx.app.inject({
        method: "POST",
        url: "/messages",
        payload: { channel: "tail", type: "info", payload: `m${i}` },
      });
    }

    const get = await ctx.app.inject({
      method: "GET",
      url: "/messages/tail?since=0&limit=3",
    });
    const msgs = get.json();
    assert.equal(msgs.length, 3);
    assert.equal(msgs[0].payload, "m4");
    assert.equal(msgs[1].payload, "m5");
    assert.equal(msgs[2].payload, "m6");
  });
