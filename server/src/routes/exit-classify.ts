...
import {
  classifyExit,
  type ClassifyExitInput,
  type ClassifyExitResult,
...
const exitClassifyPlugin: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
      ...
      schema: {
        ...
        body: {
          ...
          properties: {
            ...
            // Optional: process exit code. Older containers may not send it.
            // Range covers raw OS codes (0–255) plus shell signal+128 conventions.
    ...
    async (request): Promise<ClassifyExitResult> => {
      ...
      const { logTail, elapsedSeconds, outputLineCount, exitCode } =
        request.body;
      return classifyExit({
        logTail,
        elapsedSeconds,
