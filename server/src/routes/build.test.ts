 ...
 */
async function createBuildTestContext(
  tmpDir: string,
  ...
  const cleanup = async () => {
    ...
    try {
      ...
      /* temp dir cleanup -- safe to ignore */
    }
  };
 ...
 */
function createGitTestInfrastructure(
  tmpDir: string,
  opts?: {
...
describe("build route branch resolution", () => {
  ...
  it("defaults to docker/default/current-root when no agent is registered", async () => {
    ...
    assert.equal(
      body.success,
      false,
      "build should fail since the branch does not exist",
    );
    assert.ok(
  ...
  it("defaults to docker/default/current-root when no X-Agent-Name header is provided", async () => {
    ...
    assert.equal(
      body.success,
      false,
      "build should fail since there is no real git repo",
    );
    assert.ok(
      body.stderr.includes("docker/default/current-root"),
      `expected stderr to reference default branch "docker/default/current-root", got: ${body.stderr}`,
  ...
  it("uses agent-specific branch for /test endpoint too", async () => {
    ...
    assert.equal(
      body.success,
      false,
      "test should fail since the branch does not exist",
    );
    assert.ok(
      body.stderr.includes("docker/default/test-agent"),
      `expected stderr to reference agent branch "docker/default/test-agent", got: ${body.stderr}`,
    ...
    it("returns true when output contains the contention marker", () => {
      assert.equal(
        isUbtContentionResult({
          ...
          exit_code: 1,
          output:
            "Mutex already set, indicating that a conflicting instance of UBT is running",
          stderr: "",
        }),
        true,
      );
    ...
    it("returns true when both output and stderr contain the marker", () => {
      assert.equal(
        isUbtContentionResult({
          ...
          exit_code: 1,
          output:
            "Global\\UnrealBuildTool_Mutex already set, indicating that a conflicting instance is running",
          stderr:
            "ERROR: Mutex already set, indicating that a conflicting instance of UBT is running",
        }),
        true,
      );
    });
    ...
    it("returns true when the marker is embedded in a longer message", () => {
      assert.equal(
        isUbtContentionResult({
          ...
          output: "",
          stderr:
            "ERROR: Global\\UnrealBuildTool_Mutex_LargeProject was already set, indicating that a conflicting instance of UnrealBuildTool is already running. Aborting.",
        }),
        true,
      );
    });
    ...
    it("returns true for real UBT ConflictingInstance output", () => {
      assert.equal(
        isUbtContentionResult({
          ...
          exit_code: 1,
          output:
            "A conflicting instance of Global\\UnrealBuildTool_Mutex_bbd244c9f44ece8134630190d834139edc42379d is already running.\n\nResult: Failed (ConflictingInstance)\nTotal execution time: 0.48 seconds",
          stderr: "",
        }),
        true,
      );
...
describe("build route staging worktree sync", () => {
  ...
  function sharedContent(): string {
    ...
    for (let i = 0; i < 50; i++) {
      lines.push(
        `// Shared boilerplate line ${i}: this content is identical in both files.`,
      );
    }
    return lines.join("\n");
  }
  ...
  beforeEach(async () => {
    ...
    const git = createGitTestInfrastructure(tmpDir, {
      seedSetup: (seedDir, bareRepoDir) => {
        ...
        execFileSync("git", ["clone", bareRepoDir, "seed"], {
        ...
        execSync('git commit -m "initial commit with old file"', {
          cwd: seedDir,
          stdio: "ignore",
        });

        // Push to docker/default/current-root and docker/default/test-agent
        execSync("git checkout -b docker/default/current-root", {
          cwd: seedDir,
          stdio: "ignore",
        });
        execFileSync(
          "git",
          ["push", bareRepoDir, "docker/default/current-root"],
          { cwd: seedDir, stdio: "ignore" },
        );
        execSync("git checkout -b docker/default/test-agent", {
          cwd: seedDir,
          stdio: "ignore",
        });
        execFileSync(
          "git",
          ["push", bareRepoDir, "docker/default/test-agent"],
          { cwd: seedDir, stdio: "ignore" },
        );
      },
...
`,
  ...
  it("removes the old file when git detects a rename", async () => {
    ...
    execSync("git checkout docker/default/test-agent", {
      cwd: seedDir,
      stdio: "ignore",
    });
    ...
    execSync("git add -A", { cwd: seedDir, stdio: "ignore" });
    execSync(
      'git commit -m "rename: delete old, add new with similar content"',
      {
        cwd: seedDir,
        stdio: "ignore",
      },
