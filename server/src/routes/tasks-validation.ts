...
export async function validateSourcePath(opts: {
  ...
  if (sourcePath.includes('..') || sourcePath.startsWith('/') || sourcePath === '') {
    return { valid: false, message: `${prefix}Invalid sourcePath: ${sourcePath}` };
  }
  ...
  try {
    ...
    return { valid: false, message: `${prefix}Unknown project: "${projectId}"` };
  }
  ...
  if (bareRepo) {
    ...
    if (!existsInBareRepo(bareRepo, seedBranch, sourcePath)) {
      ...
      if (autoSync) {
        ...
        if (exteriorRepo) {
          syncExteriorToBareRepo(exteriorRepo, bareRepo, seedBranch, log);
        }
      }
