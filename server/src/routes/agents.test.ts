...
describe("agents routes (drizzle)", () => {
  ...
  it("POST /agents/register creates an agent, GET /agents returns it", async () => {
    ...
    assert.ok(
      typeof regBody.sessionToken === "string",
      "sessionToken is returned",
    );
  ...
  it("DELETE /agents soft-deletes all agents for the project", async () => {
    ...
    const rows = await ctx.db
      .select()
      .from(agents)
      .where(eq(agents.projectId, "default"));
...
describe("schema hardening V2.5 regressions", () => {
  ...
  it("cross-project coexistence: same agent name in two projects", async () => {
    ...
    const alphaRows = await ctx.db
      .select()
      .from(agents)
      .where(and(eq(agents.id, alphaId)));
    assert.equal(alphaRows[0].projectId, PROJECT_ALPHA);
    const betaRows = await ctx.db
      .select()
      .from(agents)
      .where(and(eq(agents.id, betaId)));
  ...
  it("cross-project DELETE isolation: deleting in alpha does not affect beta tasks", async () => {
    ...
    const alphaRow = await ctx.db
      .select()
      .from(agents)
      .where(
        and(eq(agents.name, "agent-1"), eq(agents.projectId, PROJECT_ALPHA)),
      );
    assert.equal(alphaRow[0].status, "deleted");

    // Beta's agent should be untouched
    const betaRow = await ctx.db
      .select()
      .from(agents)
      .where(
        and(eq(agents.name, "agent-1"), eq(agents.projectId, PROJECT_BETA)),
      );
...
describe("POST /agents/:name/sync (drizzle)", () => {
  ...
  function initBareRepoWithBranch(
    dir: string,
    branchName: string,
  ): { repo: string; initSha: string } {
    const repo = path.join(dir, "test.git");
    execSync(`git init --bare "${repo}"`);
    const emptyTree = "4b825dc642cb6eb9a060e54bf8d69288fbee4904";
    const initSha = execSync(
      `git -C "${repo}" commit-tree ${emptyTree} -m "init"`,
      { encoding: "utf-8" },
    ).trim();
  ...
  function writeFileToBranch(
    ...
    const commitSha = execSync(
      `git -C "${repo}" commit-tree ${treeSha} -p ${parentSha} -m "add ${filePath}"`,
      { encoding: "utf-8" },
    ).trim();
  ...
  beforeEach(async () => {
    ...
    const { repo, initSha } = initBareRepoWithBranch(
      tmpDir,
      "docker/default/current-root",
    );
    tmpBareRepo = repo;

    // Create agent branch from the same initial commit
    execSync(
      `git -C "${tmpBareRepo}" update-ref refs/heads/docker/default/test-agent ${initSha}`,
    );
  ...
  it("merges docker/default/current-root into docker/default/{name}", async () => {
    ...
    const content = execSync(
      `git -C "${tmpBareRepo}" show docker/default/test-agent:plan.md`,
      { encoding: "utf-8" },
    );
