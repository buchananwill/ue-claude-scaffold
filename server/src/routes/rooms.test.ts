...
import { rooms, roomMembers, agents as agentsTable, projects } from '../schema/tables.js';
import { eq } from 'drizzle-orm';
import roomsPlugin from './rooms.js';
import agentsPlugin from './agents.js';

/** Register an agent via the route and return its id */
async function registerAgent(ctx: DrizzleTestContext, name: string, projectId: string = 'default'): Promise<string> {
...
async function createRoom(
  ...
  opts: { id: string; name: string; type: string; members?: string[]; agent?: string; projectId?: string },
) {
...
describe('rooms CRUD', () => {
  ...
  it('GET /rooms returns all rooms with memberCount', async () => {
    ...
    const testRooms = roomsList.filter((r: { id: string }) => r.id === 'r1' || r.id === 'r2');
...
describe('chat messages', () => {
  ...
  it('GET /rooms/:id/messages — returns messages in ascending order', async () => {
    await ctx.app.inject({
      ...
      payload: { content: 'first' }, headers: { 'x-agent-name': 'alice', 'x-project-id': 'default' },
    });
    await ctx.app.inject({
      method: 'POST', url: '/rooms/r1/messages',
      payload: { content: 'second' }, headers: { 'x-agent-name': 'bob', 'x-project-id': 'default' },
    });
  ...
  it('GET /rooms/:id/messages?since=N returns messages after ID N', async () => {
    ...
    await ctx.app.inject({
      method: 'POST', url: '/rooms/r1/messages',
      payload: { content: 'third' }, headers: { 'x-agent-name': 'alice', 'x-project-id': 'default' },
    });
  ...
  it('GET /rooms/:id/messages — limit parameter works', async () => {
    for (let i = 0; i < 5; i++) {
      await ctx.app.inject({
        method: 'POST', url: '/rooms/r1/messages',
        payload: { content: `msg-${i}` }, headers: { 'x-agent-name': 'alice', 'x-project-id': 'default' },
      });
    }
...
describe('Option D agent/operator authorship', () => {
  ...
  it('agent-authored message, is member', async () => {
    ...
    const msg = messages.find((m: { content: string }) => m.content === 'hello from agent');
    assert.ok(msg);
  ...
  it('operator-authored message stores author_type=operator, sender=user', async () => {
    ...
    const messages = getRes.json();
    const opMsg = messages.find((m: { content: string }) => m.content === 'operator message');
    assert.ok(opMsg);
