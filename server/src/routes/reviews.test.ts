...
describe('reviews routes', () => {
  ...
  it('inserts a run plus N findings atomically', async () => {
    ...
    assert.equal((runs as unknown as { rows: Array<{ c: number }> }).rows[0].c, 1);
    const findings = await ctx.db.execute(sql`SELECT COUNT(*)::int AS c FROM review_findings WHERE run_id = ${json.runId}`);
    assert.equal((findings as unknown as { rows: Array<{ c: number }> }).rows[0].c, 2);
  });
  ...
  it('allows different reviewerRole on same (taskId, cycle)', async () => {
    ...
    const r1 = await postReview(id, {
      cycle: 1, reviewerRole: 'safety', verdict: 'approve', rawMarkdown: 'a', findings: [],
    });
    assert.equal(r1.statusCode, 200);

    const r2 = await postReview(id, {
      cycle: 1, reviewerRole: 'correctness', verdict: 'approve', rawMarkdown: 'b', findings: [],
    });
  ...
  it('allows same reviewerRole on a different cycle', async () => {
    ...
    const r1 = await postReview(id, {
      cycle: 1, reviewerRole: 'safety', verdict: 'request_changes', rawMarkdown: 'a', findings: [],
    });
    assert.equal(r1.statusCode, 200);

    const r2 = await postReview(id, {
      cycle: 2, reviewerRole: 'safety', verdict: 'approve', rawMarkdown: 'b', findings: [],
    });
    assert.equal(r2.statusCode, 200);
  });
  ...
  it('rejects missing cycle', async () => {
    const id = await createTask();
    const res = await postReview(id, {
      reviewerRole: 'safety', verdict: 'approve', rawMarkdown: '', findings: [],
    });
  ...
  it('rejects bad verdict', async () => {
    const id = await createTask();
    const res = await postReview(id, {
      cycle: 1, reviewerRole: 'safety', verdict: 'maybe', rawMarkdown: '', findings: [],
    });
    assert.equal(res.statusCode, 400);
  });
  ...
  it('rejects bad reviewerRole', async () => {
    const id = await createTask();
    const res = await postReview(id, {
      cycle: 1, reviewerRole: 'bad role!', verdict: 'approve', rawMarkdown: '', findings: [],
    });
    assert.equal(res.statusCode, 400);
  });
  ...
  it('rejects bad severity', async () => {
    ...
    const res = await postReview(id, {
      ...
      verdict: 'request_changes',
      rawMarkdown: '',
      findings: [{ severity: 'WARNING', ordinal: 0, title: 't', description: '' }],
    });
    assert.equal(res.statusCode, 400);
  });
  ...
  it('returns runs array with correct shape', async () => {
    ...
    await postReview(id, {
      ...
      findings: [
        { severity: 'BLOCKING', ordinal: 0, title: 'finding A', description: 'd' },
      ],
    });
    await postReview(id, {
      cycle: 2,
    ...
    const safety = json.runs.find((r: { reviewerRole: string }) => r.reviewerRole === 'safety');
    assert.ok(safety);
    ...
    const correctness = json.runs.find((r: { reviewerRole: string }) => r.reviewerRole === 'correctness');
  ...
  it('only returns runs from the requested cycle', async () => {
    ...
    await postReview(id, {
      cycle: 1, reviewerRole: 'safety', verdict: 'approve', rawMarkdown: 'c1', findings: [],
    });
    await postReview(id, {
      cycle: 2, reviewerRole: 'safety', verdict: 'request_changes', rawMarkdown: 'c2', findings: [],
    });
