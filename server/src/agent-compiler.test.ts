...
describe("agent-compiler", () => {
  ...
  describe("exact compiled output", () => {
    it("asserts exact full content of compiled output (frontmatter + separator + body)", () => {
      ...
      const source = writeFile(
        "dynamic/exact-agent.md",
        "---\nname: exact-agent\nmodel: opus\nskills:\n  - only-skill\n---\n\nAgent preamble.\n",
      );

      compileAgent(source, outputDir, skillsDir);
      const compiled = fs.readFileSync(
        path.join(outputDir, "exact-agent.md"),
        "utf-8",
      );
  ...
  describe("compileAgent", () => {
    it("compiles a single agent with skills", () => {
      ...
      const metaJson = JSON.parse(
        fs.readFileSync(path.join(outputDir, "test-agent.meta.json"), "utf-8"),
      );
    ...
    it("warns and copies as-is when agent has no skills", () => {
      ...
      const metaJson = JSON.parse(
        fs.readFileSync(path.join(outputDir, "no-skills.meta.json"), "utf-8"),
      );
  ...
  describe("access scope ranking", () => {
    it("highest scope wins across multiple skills", () => {
      ...
      compileAgent(source, outputDir, skillsDir);
      const metaJson = JSON.parse(
        fs.readFileSync(path.join(outputDir, "ranked.meta.json"), "utf-8"),
      );
    ...
    it("SCOPE_RANK has correct ordering", () => {
      ...
      assert.ok(
        SCOPE_RANK["write-access"] < SCOPE_RANK["ubt-build-hook-interceptor"],
      );
    });
  });
  ...
  describe("findSubAgents", () => {
    it("finds referenced dynamic agents in compiled body", () => {
      const dynamicDir = makeDir("dynamic");
      writeFile(
        "dynamic/container-implementer.md",
        "---\nname: impl\n---\n\nBody\n",
      );
      writeFile(
        "dynamic/container-reviewer.md",
        "---\nname: rev\n---\n\nBody\n",
      );
      writeFile(
        "dynamic/container-tester.md",
        "---\nname: test\n---\n\nBody\n",
      );

      const body =
        "Use container-implementer for code changes and container-tester for tests.";
    ...
    it("respects exclude set", () => {
      ...
      const result = findSubAgents(
        body,
        dynamicDir,
        new Set(["container-implementer"]),
      );
  ...
  describe("compileAgentWithSubAgents", () => {
    it("compiles the lead and one-level sub-agents referenced in a skill", () => {
      const skillsDir = makeDir("skills");
      const dynamicDir = makeDir("dynamic");
      const outputDir = path.join(tmpDir, "output");

      // Skill that names the worker — this is the source of truth for sub-agent dispatch
      writeFile(
        "skills/dispatch/SKILL.md",
        "---\nname: dispatch\n---\n\n# Dispatch\n\nDispatch each phase to worker-bee for execution.\n",
      );

      writeFile(
        "dynamic/worker-bee.md",
        "---\nname: worker-bee\nmodel: opus\n---\n\nWorker body.\n",
      );
      ...
      assert.equal(result.main.type, "lead");
      assert.equal(result.subAgents.length, 1);
      assert.equal(result.subAgents[0].type, "worker-bee");
      assert.deepEqual(result.warnings, []);

      // Both files exist on disk
      assert.ok(fs.existsSync(result.main.outputPath));
      assert.ok(fs.existsSync(result.subAgents[0].outputPath));
    });

    it("does not recurse beyond one level — emits a warning instead", () => {
      const skillsDir = makeDir("skills");
      const dynamicDir = makeDir("dynamic");
      const outputDir = path.join(tmpDir, "output");

      // The lead's skill names sub-agent A.
      writeFile(
        "skills/lead-skill/SKILL.md",
        "---\nname: lead-skill\n---\n\nLead dispatches to a-agent for work.\n",
      );
      // a-agent's skill in turn names b-agent — this would be two-level nesting.
      writeFile(
        "skills/a-skill/SKILL.md",
        "---\nname: a-skill\n---\n\nA-agent then refers work to b-agent.\n",
      );
      ...
      assert.equal(result.subAgents.length, 1);
      assert.equal(result.subAgents[0].type, "a-agent");
      assert.equal(result.warnings.length, 1);
      assert.match(result.warnings[0], /a-agent references b-agent/);
      // b-agent must not have been compiled
      assert.ok(!fs.existsSync(path.join(outputDir, "b-agent.md")));
    });

    it("returns an empty subAgents list when the lead references nothing", () => {
      const skillsDir = makeDir("skills");
      const dynamicDir = makeDir("dynamic");
      const outputDir = path.join(tmpDir, "output");
      ...
      assert.equal(result.main.type, "solo-agent");
      assert.deepEqual(result.subAgents, []);
      assert.deepEqual(result.warnings, []);
    });

    it("dedup across multiple invocations via shared excludeSet", () => {
      const skillsDir = makeDir("skills");
      const dynamicDir = makeDir("dynamic");
      const outputDir = path.join(tmpDir, "output");
      ...
      writeFile(
        "skills/dispatch/SKILL.md",
        "---\nname: dispatch\n---\n\nBoth leads dispatch to worker-bee.\n",
      );
      writeFile(
        "dynamic/worker-bee.md",
        "---\nname: worker-bee\n---\n\nWorker.\n",
      ...
      const lead1 = writeFile(
        "dynamic/lead-one.md",
        "---\nname: lead-one\nskills:\n  - dispatch\n---\n\nOne.\n",
      );
      const lead2 = writeFile(
        "dynamic/lead-two.md",
        "---\nname: lead-two\nskills:\n  - dispatch\n---\n\nTwo.\n",
      );

      const exclude = new Set<string>();
      const r1 = compileAgentWithSubAgents(
        lead1,
        outputDir,
        skillsDir,
        dynamicDir,
      ...
      const r2 = compileAgentWithSubAgents(
        lead2,
        outputDir,
        skillsDir,
        dynamicDir,
      ...
      assert.equal(r1.subAgents.length, 1);
      assert.equal(r1.subAgents[0].type, "worker-bee");
      // Second invocation must NOT recompile worker-bee — already in exclude
      assert.deepEqual(r2.subAgents, []);
    });
  });
});
